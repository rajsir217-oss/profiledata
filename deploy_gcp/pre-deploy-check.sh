...
if grep -q "ENVIRONMENT: 'local'" "$PROJECT_ROOT/frontend/public/config.js" 2>/dev/null; then
    ...
    ((ERRORS++))
elif grep -q "ENVIRONMENT: 'pod'" "$PROJECT_ROOT/frontend/public/config.js" 2>/dev/null; then
    echo -e "${GREEN}   ✅ PASS: config.js is set to 'pod'${NC}"
else
    echo -e "${YELLOW}   ⚠️  WARNING: Unknown environment or file not found${NC}"
    ((WARNINGS++))
fi

# Check 2: Backend .env file
echo -e "${BLUE}2. Checking backend .env configuration...${NC}"
if [ -f "$PROJECT_ROOT/fastapi_backend/.env" ]; then
    if grep -q "USE_GCS=False" "$PROJECT_ROOT/fastapi_backend/.env"; then
...
if [ -f "$PROJECT_ROOT/fastapi_backend/.env.production" ]; then
    echo -e "${GREEN}   ✅ PASS: .env.production exists${NC}"
    
    # Check required fields
    if ! grep -q "MONGODB_URL=mongodb+srv" "$PROJECT_ROOT/fastapi_backend/.env.production"; then
...
CONSOLE_COUNT=$(grep -r "console\.log" "$PROJECT_ROOT/frontend/src" --include="*.js" --include="*.jsx" 2>/dev/null | grep -v "NODE_ENV" | wc -l | tr -d ' ')
...
LOCALHOST_COUNT=$(grep -r "localhost:8000\|localhost:3000" "$PROJECT_ROOT/frontend/src" --include="*.js" --include="*.jsx" 2>/dev/null | grep -v "RUNTIME_CONFIG" | grep -v "process.env" | grep -v "//" | wc -l | tr -d ' ')
...
if [ -f "$PROJECT_ROOT/fastapi_backend/requirements.txt" ]; then
...
if [ -f "$PROJECT_ROOT/fastapi_backend/Dockerfile" ]; then
