...
echo -e "${GREEN}✅ .env.local set to production (from .env.android)${NC}"
...
EOF
echo -e "${GREEN}✅ Production config.js written (Backend: ${BACKEND_URL})${NC}"
echo ""
...
if [ -f "public/config.js.dev.backup" ]; then
    ...
    echo -e "${GREEN}✅ Development config.js restored${NC}"
else
    cat > public/config.js << EOF
/**
...
console.log('✅ Runtime config loaded for', window.RUNTIME_CONFIG.ENVIRONMENT, '| Log Level:', window.RUNTIME_CONFIG.LOG_LEVEL);
EOF
    echo -e "${GREEN}✅ Development config.js created${NC}"
fi
