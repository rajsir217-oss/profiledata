...
| Metric | Counts |
|---|---|
| `redis_max_clients` | Logs containing `max number of clients reached` |
| `backend_errors` | All `severity>=ERROR` logs |
...
| High request latency (p95) | p95 > 2000ms for 5m | built-in `run.googleapis.com/request_latencies` |
