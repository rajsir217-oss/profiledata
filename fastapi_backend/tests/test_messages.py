...
class TestSendMessage:
    ...
    def test_send_message_to_nonexistent_user(self, test_users, cleanup_messages):
...
class TestMessageEdgeCases:
    ...
    def test_get_conversation_nonexistent_user(self, test_users, cleanup_messages):
        """Test getting conversation with non-existent user"""
        response = client.get("/api/messages/conversation/nonexistent?username=user1")
        
        # Should return empty conversation or 404
        assert response.status_code in [200, 404]
    
    def test_concurrent_message_sending(self, test_users, cleanup_messages):
...
class TestMessageIntegration:
    ...
    def test_full_conversation_workflow(self, test_users, cleanup_messages):
        ...
        send_response2 = client.post(
            "/api/messages/send?username=user2",
            json={"toUsername": "user1", "content": "Hi there!"}
        )
        assert send_response2.status_code == 200
        
        # Get conversation as user1
        conv_response = client.get("/api/messages/conversation/user2?username=user1")
        assert conv_response.status_code == 200
        assert len(conv_response.json()["messages"]) == 2
        
        # Get conversations list
        list_response = client.get("/api/messages/conversations?username=user1")
