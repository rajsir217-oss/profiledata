...
class PlatformStatsYearlyAggregationTemplate(JobTemplate):
    """Job template for yearly platform stats aggregation"""
    
    template_type = "platform_stats_yearly_aggregation"
    template_name = "Platform Stats Yearly Aggregation"
    ...
    async def execute(self, context: JobExecutionContext) -> JobResult:
        ...
        try:
            ...
            return JobResult(
                status="failed",
                message=f"Failed to aggregate yearly stats for {year_id}: {str(e)}",
                errors=[str(e)]
            )
