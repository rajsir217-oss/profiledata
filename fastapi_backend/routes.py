...
# Includes all fields needed by SearchProfileCard component
...
async def update_user_preferences(
    ...
    # Validate theme
    valid_themes = ['light-blue', 'sky-blue', 'dark', 'light-pink', 'light-gray', 'ultra-light-gray', 'ultra-light-green', 'ultra-black', 'indian-wedding', 'newspaper', 'cute-bubble', 'white']
...
async def close_conversation(
    ...
    - Sends graceful notification to other user
    - Does NOT add to exclusion list (Phase 1)
    """
    username = current_user["username"]
    logger.info(f"🚪 Closing conversation: {username} declining {other_username}")
    ...
    try:
        ...
        try:
            ...
            logger.info(f"📤 Sent closure notification to {other_username}")
            notification_sent = True
        except Exception as notif_err:
            logger.warning(f"⚠️ Failed to send closure notification: {notif_err}")
        ...
        try:
            ...
            await activity_logger.log_activity(
                username=username,
                action_type=ActivityType.CONVERSATION_ARCHIVED,
                target_username=other_username,
                metadata={"action": "declined", "reason": "not_interested"}
            )
        except Exception as log_err:
            logger.warning(f"⚠️ Failed to log activity: {log_err}")
        
        logger.info(f"✅ Conversation closed: {username} -> {other_username}")
        ...
        logger.error(f"❌ Error closing conversation: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_conversation_status(
    ...
    """
    Get the status of a conversation (active, closed, etc.)
    """
    username = current_user["username"]
    
    try:
        status_doc = await db.conversation_status.find_one({
