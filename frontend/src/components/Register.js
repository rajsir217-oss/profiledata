      ...
      <form onSubmit={handleSubmit} encType="multipart/form-data">
        ...
        {Object.entries(formData).map(([key, value]) => {
          ...
          const excludedFields = [
            ...
            // Exclude aboutYou and partnerPreference (rendered with sample carousel below)
            "aboutYou", "partnerPreference"
          ];
          
          if (excludedFields.includes(key)) return null;
          
          if (["education", "familyBackground", "workplace"].includes(key)) {
            return (
        ...
        <div className="mb-3">
          ...
          <textarea
            className={`form-control ${fieldErrors.aboutYou && touchedFields.aboutYou ? 'is-invalid' : ''}`}
            name="aboutYou"
            ...
            required
          />
          {fieldErrors.aboutYou && touchedFields.aboutYou && (
            <div className="invalid-feedback d-block">{fieldErrors.aboutYou}</div>
          )}
        </div>
