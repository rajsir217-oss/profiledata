...
const Register3 = ({ mode = 'register', editUsername = null }) => {
  ...
  const handleTabAutoSave = async (tabId) => {
    ...
    try {
      ...
      localStorage.setItem('register3Draft', JSON.stringify(draftData));
    } catch (error) {
  ...
  useEffect(() => {
    if (!isEditMode) {
      try {
        const draft = localStorage.getItem('register3Draft');
        if (draft) {
  ...
  const handleDiscardDraft = () => {
    localStorage.removeItem('register3Draft');
    setShowDraftModal(false);
  };
    ...
    <div className={` ${isEditMode ? 'edit-mode' : 'registration-mode'}`} style={{
      ...
      {(isEditMode || registrationOpen || invitationToken) && <div style={{
        ...
        {/* <div className="card p-4 shadow" style={{ */}
          ...
          {/* Back to Profile — absolute top-right of card */}
          {isEditMode && (
            <button 
              type="button" 
      ...
      <form onSubmit={handleSubmit} encType="multipart/form-data" className="register-container">
        ...
        <div className="register3-section">
          <div className="register3-section-header"><h4>👤 PERSONAL INFORMATION</h4></div>
        ...
        <div className="row mb-3 align-items-start">
          <div className="col-md-4">
            ...
            <small className="text-muted">
              ℹ️ Your age is calculated from month/year.
            </small>
          </div>
          <div className="col-md-4">
            <label className="form-label">
        ...
        <div className="row mb-3">
                ...
                <div key={index} className="contact-number-row">
                  <div className="contact-number-fields">
                    ...
                    >
                      {CONTACT_LABELS.map(label => (
                        <option key={label} value={label}>
                          {isMobileView ? CONTACT_LABEL_ABBR[label] : label.charAt(0).toUpperCase() + label.slice(1)}
                        </option>
                      ))}
                    </select>
                    <input
                    ...
                    <button
                      ...
                      title={contact.visible !== false ? "Visible to members — click to hide" : "Hidden from members — click to show"}
                      onClick={() => handleContactNumberChange(index, 'visible', !(contact.visible !== false))}
                    >
          ...
          <div className="col-12">
            ...
            <div className="linkedin-url-row">
              ...
              />
              {/* Visibility Toggle for Contact Email - eye emoji button */}
              <button
                type="button"
                className="btn-visibility-toggle"
                title={formData.contactEmailVisible !== false ? "Visible to members — click to hide" : "Hidden from members — click to show"}
                onClick={() => {
                  const value = !(formData.contactEmailVisible !== false);
              ...
              >
                {formData.contactEmailVisible !== false ? '👁️' : '🙈'}
              </button>
            </div>
        ...
        {/* Mother Tongue + Eating Preference */}
        <div className="row mb-3">
          <div className="col-md-6">
        ...
        {/* Languages Spoken (Religion moved to Optional section) */}
        <div className="row mb-3">
          <div className="col-md-6">
        ...
        {/* Family Background - moved here below Bio/Tagline */}
        <TextAreaWithSamples
        ...
        <div className="row mb-3">
          <div className="col-12">
            ...
            <div className="linkedin-url-row">
              ...
              />
              {/* Visibility Toggle for LinkedIn URL - eye emoji button */}
              <button
                type="button"
                className="btn-visibility-toggle"
                title={formData.linkedinUrlVisible !== false ? "Visible to members — click to hide" : "Hidden from members — click to show"}
                onClick={() => {
                  const value = !(formData.linkedinUrlVisible !== false);
              ...
              >
                {formData.linkedinUrlVisible !== false ? '👁️' : '🙈'}
              </button>
            </div>
          </div>
        </div>
        ...
        {Object.entries(formData).map(([key, value]) => {
          ...
          const excludedFields = [
            ...
            // Exclude aboutMe, partnerPreference, partnerCriteria, and familyBackground (rendered explicitly)
            "aboutMe", "partnerPreference", "partnerCriteria", "familyBackground",
        ...
        {/* My Preferences & Lifestyle - Moved to Optional section */}
        <div className="lifestyle-section my-lifestyle mt-4" style={{
          ...
          <h5 className="mb-3" style={{ color: 'var(--primary-color)', fontWeight: '600' }}>
            👤 My Preferences & Lifestyle
          </h5>
          <p className="text-muted small mb-3">Tell us about yourself - these help find compatible matches</p>
          

          {/* My Caste + My Religion */}
          <div className="row mb-3">
            <div className="col-md-6">
              <label className="form-label">My Caste <span className="text-muted">(Optional)</span></label>
              <input
                type="text"
                className="form-control"
                name="caste"
            ...
            <div className="col-md-6">
              <label className="form-label">My Religion <span className="text-muted">(Optional)</span></label>
              <select
        ...
        <div className="lifestyle-section partner-pref-section mt-4" style={{
          background: 'linear-gradient(135deg, rgba(236, 72, 153, 0.08) 0%, rgba(249, 168, 212, 0.05) 100%)',
        ...
        */}
        ...
        {/* End Partner Preferences Section */}
        ...
        <div className="register3-section">
          <div className="register3-section-header"><h4>📋 ACKNOWLEDGEMENTS</h4></div>
                <div className="tab-section">
