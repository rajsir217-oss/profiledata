...
const EmailTemplatePreview = () => {
  ...
  const renderSampleData = (body) => {
    ...
    // Supports both dot notation ({match.firstName}) and underscore notation ({match_firstName})
    if (!body) {
      return '<div style="padding: 20px; text-align: center;">No template body available</div>';
    }
    
    return body
      // Recipient variables (both formats)
      .replace(/{recipient\.firstName}/g, 'John')
      .replace(/{recipient_firstName}/g, 'John')
      .replace(/{recipient\.lastName}/g, 'Doe')
      .replace(/{recipient_lastName}/g, 'Doe')
      // Match/Actor variables (both formats)
      ...
      .replace(/{match_age}/g, '28')
      .replace(/{match\.location}/g, 'San Francisco, CA')
      .replace(/{match_location}/g, 'San Francisco, CA')
      .replace(/{match\.profession}/g, 'Software Engineer')
      .replace(/{match_profession}/g, 'Software Engineer')
      .replace(/{match\.matchScore}/g, '85')
      .replace(/{match_matchScore}/g, '85')
      .replace(/{match\.username}/g, 'sarahsmith')
      .replace(/{match_username}/g, 'sarahsmith')
      // Status template variables (lowercase)
      ...
      .replace(/{contact_email}/g, 'sarah.smith@example.com')
      ...
      .replace(/{login_location}/g, 'San Francisco, CA')
      .replace(/{login\.device}/g, 'Chrome on Mac OS')
      .replace(/{login_device}/g, 'Chrome on Mac OS')
      .replace(/{login\.timestamp}/g, new Date().toLocaleString())
      .replace(/{login_timestamp}/g, new Date().toLocaleString())
      .replace(/{login\.ipAddress}/g, '192.168.1.1')
      .replace(/{login_ipAddress}/g, '192.168.1.1')
      // Milestone variables
      ...
      .replace(/{profile_missingFields}/g, 'Photos, Education')
      ...
      .replace(/{request_type}/g, 'contact_info')
      // URL variables (underscore format used in templates)
      .replace(/{dashboard_url}/g, '#dashboard')
      .replace(/{profile_url}/g, '#profile')
      .replace(/{chat_url}/g, '#chat')
      .replace(/{search_url}/g, '#search')
      .replace(/{contact_url}/g, '#contact')
      .replace(/{security_url}/g, '#security')
      .replace(/{reset_url}/g, '#reset-password')
      .replace(/{preferences_url}/g, '#preferences')
      .replace(/{unsubscribe_url}/g, '#unsubscribe')
      .replace(/{tracking_pixel_url}/g, `${getBackendUrl()}/api/email-tracking/pixel/preview`)
      // App variables (dot format)
